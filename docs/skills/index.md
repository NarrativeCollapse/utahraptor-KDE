# Skill catalog (generated)

This file is a human-readable mirror of `index.json`. Both are generated by
`scripts/generate_skill_index.py` — do not hand-edit either file.

schema 1.0 · 9 skills

| id | category | status | one-line purpose |
|---|---|---|---|
| [ci-workflows](ci-workflows.md) | ci-ops | active | Navigate Utah's build, promote, and sync workflow topology. |
| [containerfile](containerfile.md) | image-build | active | Edit the Containerfile without regressing layer count or cache hits. |
| [desktop-contract](desktop-contract.md) | contracts | active | Maintain OS identity, Plasma desktop defaults, and first-boot Flatpak policy. |
| [flavors](flavors.md) | image-build | active | Add, remove, or retire an image flavor safely. |
| [kernel-cache](kernel-cache.md) | image-build | active | Understand and rebuild the OGC kernel and NVIDIA module cache image. |
| [local-testing](local-testing.md) | testing | active | Build, install, and boot Utah locally in a VM or live ISO. |
| [package-contract](package-contract.md) | contracts | active | Maintain Bluefin package parity and Utah's overlay manifest. |
| [plasma-migration](plasma-migration.md) | contracts | active | Plan and measure the migration of Utah's desktop from GNOME to KDE Plasma. |
| [skill-improvement](skill-improvement.md) | meta | active | Capture durable agent learnings in maintained docs. |
