#!/usr/bin/env python3
"""Assert that Utah actually contains its Bluefin and Plasma 6 RPM contracts.
...
def main() -> int:
    ...
    if resolved.exists():
        ...
        plasma_names = set(section(overlay, "plasma"))
        ...
        overlay_names = plasma_names | parity_names | hardware_names | service_names
        bluefin = [p for p in contract if p not in overlay_names]
        plasma = [p for p in contract if p in plasma_names]
    ...
    expected = [*bluefin, *plasma, *parity, *hardware, *services, *nvidia]

    print(
        f"Verifying {len(bluefin)} Bluefin packages, {len(plasma)} Plasma desktop packages,"
